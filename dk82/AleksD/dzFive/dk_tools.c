...
int addElement(Array *anArray, Trapeze *aTrapeze)
	{
		...
		if (NULL != anArray && NULL != aTrapeze && anArray->counter < anArray->number)
			{
				anArray->trapezes[anArray->counter] = aTrapeze;
		...
		if (NULL != anArray && NULL != aTrapeze && index >= 0 && index < anArray->number)
			{
				if (NULL == anArray->trapezes[index])
					{
						anArray->trapezes[index] = aTrapeze;
						anArray->counter++;
					}
				else
					{
						destroyAndAddTrapezeByIndex(anArray, aTrapeze, index);
					}
				theResult = index;
			}
		return theResult;
...
void destroyAndAddTrapezeByIndex(Array *anArray, Trapeze* aTrapeze, int index)
	{
		if (NULL != aTrapeze)
		{		
...
void writePointToJSON(FILE *fo, Point *aPoint)
	...
	int binarySearch(Array* anArray ,  int value)
{
	int max = anArray -> counter;
	int min = 0;
	while(min <= max)
	{
		int mid = (min + max) / 2;
		
		if (value == squareTrapeze(anArray->trapezes[mid]))
		{
		return mid;

		}
		else if(value > squareTrapeze(anArray->trapezes[mid]))
		{
			min = mid+1;
		}
		else
