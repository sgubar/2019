...
void printToFileTrapezium(FILE *file, Trapezium *trapezium) {
	...
	fprintf(file,"{\n	\"Trapezium\":(Trapezium){\n[");
		...
		fprintf(file, "			\"dot\:{\n				\"id\":(char) = \"%c\",\n				\"x\":(integer) =\"%i\",\n				\"y\":(integer) =\"%i\"\n},\n", trapezium->leftBottomCorner->id,trapezium->leftBottomCorner->x, trapezium->leftBottomCorner->y);
		fprintf(file, "			\"dot\:{\n				\"id\":(char) = \"%c\",\n				\"x\":(integer) =\"%i\",\n				\"y\":(integer) =\"%i\"\n},\n", trapezium->leftTopCorner->id, trapezium->leftTopCorner->x, trapezium->leftTopCorner->y);
		fprintf(file, "			\"dot\:{\n				\"id\":(char) = \"%c\",\n				\"x\":(integer) =\"%i\",\n				\"y\":(integer) =\"%i\"\n},\n", trapezium->rightTopCorner->id, trapezium->rightTopCorner->x, trapezium->rightTopCorner->y);
		fprintf(file, "			\"dot\:{\n				\"id\":(char) = \"%c\",\n				\"x\":(integer) =\"%i\",\n				\"y\":(integer) =\"%i\"\n}",    trapezium->rightBottomCorner->id, trapezium->rightBottomCorner->x, trapezium->rightBottomCorner->y);


	fprintf(file, "],\n		\"AB\":%2f,\n		\"BC\":%2f,\n		\"CD\":%2f,\n		\"DA\":%2f,\n	\"square\":%2f\n}\n}\n",trapezium->ab,trapezium->bc,trapezium->cd,trapezium->da,trapezium->square);
}
